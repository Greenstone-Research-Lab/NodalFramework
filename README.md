...
To inspect the complete production output—including the generated DocFX site under `/api`—use the Cloudflare-compatible preview instead of `docusaurus serve`:

```powershell
npm.cmd run preview --prefix website
```

In a second terminal, verify the homepage, integrated `/api/` base path, DocFX stylesheet, runtime, and logo with:

```powershell
npm.cmd run smoke:preview --prefix website
```
